package app.te.protein_chef.presentation.auth.social
...
class SocialHelper {
  ...
  fun googleSignResult(data: Intent?): UpdateProfileRequest {
    try {
  ...
  fun setUpFacebook(loginButton: LoginButton): UpdateProfileRequest {
  ...
  private fun handleFacebookAccessToken(token: AccessToken): UpdateProfileRequest {
    ...
    auth.signInWithCredential(credential)
      .addOnCompleteListener { task ->
        if (task.isSuccessful) {
          ...
          if (user != null) {
            registerRequest.socialToken = user.providerId
